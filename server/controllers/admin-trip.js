...
function tripDetails(req, res, next) {
  ...
  TripSchema.getCount(filter)
    .then((totalTripRecords) => { //eslint-disable-line
      const returnObj = {
        success: false,
      ...
      TripSchema.list({ skip, limit, filter })
        .then((tripData) => {
          if (tripData.length !== 0) {
            for (let i = 0; i < tripData.length; i++) { //eslint-disable-line
...
function addRider(returnObj) {
  return new Promise((resolve, reject) => {
    Promise.all(returnObj.data.map((item, index) => (
      UserSchema.findOneAsync({ _id: item.riderId })
        .then((result) => {
...
function addDriver(returnObj) {
  return new Promise((resolve, reject) => {
    Promise.all(returnObj.data.map((item, index) => (
      UserSchema.findOneAsync({ _id: item.driverId })
        .then((result) => {
...
function createNewTrip(req, res, next) {
  ...
  UserSchema.findAsync({ $or: [{ $and: [{ userType: 'rider' }, { _id: riderId }] }, { $and: [{ userType: 'driver' }, { _id: driverId }] }] })
    .then((foundUserData) => { //eslint-disable-line
      ...
      tripObj.saveAsync()
        .then((newTripObj) => {
          ...
          tripRequest.saveAsync()
            .then(() => {
              UserSchema.updateAsync({ $or: [{ _id: newTripObj.riderId }, { _id: newTripObj.driverId }] }, { $set: { currTripId: newTripObj._id, currTripState: 'trip' } }, { multi: true }) //eslint-disable-line
                .then(() => {
                  res.send(returnObj);
                })
                .error((e) => {
...
function updateTrip(req, res, next) {
  ...
  TripSchema.findOneAndUpdateAsync({ _id: tripId }, { $set: tripObj }, { new: 1, runValidators: true })
    .then((updatedTripObj) => { //eslint-disable-line
      const returnObj = {
        success: false,
...
export default {
  tripDetails, getOngoingTripDetails, getRecentReviewedTripDetails, createNewTrip, updateTrip, loadTripDetails, tripRevenueGraph, getSpecificUserTripDetails
};
