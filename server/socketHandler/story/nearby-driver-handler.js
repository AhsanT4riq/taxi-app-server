...
function nearbyDriverHandler(socket) {
  socket.on('updatePickupRegion', userRegion => {
    ...
    UserSchema.findOneAndUpdateAsync({ _id: userId }, { $set: { mapCoordinates: coordinates } }, { new: true })
      .then(updatedUser => UserSchema.findAsync({ $and: [{ gpsLoc: { $geoWithin: { $centerSphere: [updatedUser.mapCoordinates, config.radius] } } }, { currTripId: null, currTripState: null }, { loginStatus: true }, { userType: 'driver' }, { isAvailable: true }] }))
      .then(driverArray => {
        if (driverArray) {
