...
function requestTripHandler(socket) {
        ...
        roundRobinAsync(nearByDriversDoc, quantum, payload)
          .then(result => {
            if (result === false) {
              ...
              SocketStore.emitByUserId(payload.rider._id, 'tripRequestUpdated', payload.tripRequest);
            }
          })
  ...
  function createTripRequestObjAsync(payload, driver) {
    return new Promise(resolve => {
      ...
      const boosterSeatNum = payload.tripRequest.boosterSeatNum;
      ...
      tripRequestObj
        ...
        .then(savedTripRequest => {
          ...
          UserSchema.updateAsync({ $or: [{ _id: savedTripRequest.riderId }, { _id: savedTripRequest.driverId }] }, { $set: { currTripId: savedTripRequest._id, currTripState: 'tripRequest' } }, { new: true, multi: true })
            .then(() => {
  ...
  function resetTripRequestAsync(driverObj) {
    ...
    return new Promise(resolve => {
      UserSchema.updateAsync({ $or: [{ _id: driverObj._id }] }, { $set: { currTripId: null, currTripState: null } }, { new: true, multi: true })
        .then(() => resolve())
        .error(e => {
  ...
  function nearByDriver(riderId) {
    return new Promise((resolve, reject) =>
      UserSchema.findOneAsync({ _id: riderId, userType: 'rider' })
        .then(userDoc => {
          if (userDoc) {
            return UserSchema.findAsync({ $and: [{ gpsLoc: { $geoWithin: { $center: [userDoc.gpsLoc, config.radius] } } }, { currTripId: null, currTripState: null }, { loginStatus: true }, { userType: 'driver' }] })
              .then(driverDoc => {
                if (driverDoc) {
