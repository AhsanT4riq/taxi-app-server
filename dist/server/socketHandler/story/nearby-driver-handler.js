...
function nearbyDriverHandler(socket) {
  socket.on('updatePickupRegion', function (userRegion) {
    ...
    _user2.default.findOneAndUpdateAsync({ _id: userId }, { $set: { mapCoordinates: coordinates } }, { new: true }).then(function (updatedUser) {
      return _user2.default.findAsync({ $and: [{ gpsLoc: { $geoWithin: { $centerSphere: [updatedUser.mapCoordinates, _env2.default.radius] } } }, { currTripId: null, currTripState: null }, { loginStatus: true }, { userType: 'driver' }, { isAvailable: true }] });
