...
TripSchema.path('riderId').validate(function (riderId, respond) {
  debug('inside validator with riderId value ->' + riderId);
  return _user2.default.findByIdAsync(riderId).then(function (riderData) {
    if (riderData) {
      return respond(true);
    } else {
...
TripSchema.path('driverId').validate(function (driverId, respond) {
